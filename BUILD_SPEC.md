...
> **Addendum (post-build, sourced from a real gameplay screenshot and a
> subsequent tape-image extraction, not fresh disassembly):** the render
> model is now fully recovered. The map cell byte is a **full-byte tile
> index**: rendering looks up a 256-entry ZX attribute table (0xD80E) and
> 8-byte tile bitmaps (0xF6D8) by the whole byte; §3.1's "type in low
> nibble" is the *game-logic* mask only. Sea's blue is INK blue over
> yellow PAPER via a fully-inked tile; open desert is a blank tile (pure
> paper); escarpment art is yellow paper / red ink. The recovery was
> validated three ways: a 256x192 gameplay screenshot (which also
> independently confirmed the 22x22 viewport, the 2x2 counter footprint,
> and exact nation-ink RGB values — German pure black, British non-bright
> blue `(0,0,162)`, Italian `(231,0,182)`), a full-map reconstruction
> matching pubmap_units.png at 94.8% sea-mask agreement, and blank/solid
> sanity tiles. See `data/render_model.json`,
> `reference/extraction_tools/extract_render_tables.py`,
> `reference/screenshot_evidence.md`, and NOTES.md. Tile bitmaps are
> original pixel art and are kept out of the committed repo (local-only,
> regenerated by the extraction script).

> **Second addendum (travel/combat 1:1 audit + deployment recovery, from
> tape disassembly of the resolvers -- full account in NOTES.md):**
> * **Terrain typing correction (§3.1):** a cell's terrain TYPE is
>   `type_table[cell_byte] & 15` via a 256-entry table at 0xD90E — NOT the
>   cell byte's own low nibble, which misclassifies 2011/3200 cells. The
>   code-verified type space is 0–8: 0 desert (incl. decorative art),
>   1 sea, 4 escarpment, 5 road, 6 marsh, plus small unknown types
>   2/3/7/8. `data/terrain_logic.json` supersedes
>   `data/terrain_authentic.json`'s grid for game logic. A road-DIRECTION
>   table (0xDA0E) gating the road cost discount by step direction is
>   located but its arithmetic is not yet applied in the clone.
> * **Travel (§5.1–5.2) verified 1:1:** Assault ×1.5 and Travel ×0.5 are
>   the literal cost transforms in the mover; the road test is exactly
>   `type == 5`; the footprint scan iterates 2×2 and collapses to 1×1 on
>   the travel flag.
> * **Supply (§5.4) index corrected:** the curve index is
>   `(distance + 2) >> 2`, not `distance >> 2`.
> * **Combat (§5.5) model replaced:** the "+3 byte" is a per-unit
>   combat-PRESSURE accumulator (starts 0 at placement, 8-bit
>   saturating). Resolution: `value = pressure × 100 / strength` tested
>   against a threshold = the unit's MORALE (or the fixed 20 for
>   combat-class-10 units); at/above it the unit takes −10 efficiency, is
>   forced to HOLD and attempts a one-cell retreat; if trapped, pressure
>   escalates ×1.5 (cap 255); pressure ≥ strength routes to a break path
>   (semantics open). The pressure INFLOW rate, out-of-contact decay and
>   retreat-direction order remain inferred/tunable; the −20
>   caught-on-road doubling has NO distinct call site in the audit and is
>   retained provisionally.
> * **Deployment (§5.6) recovered:** each scenario has a scripted
>   deployment list (`data/deployments.json`) placing its starting units
>   at historical positions — divisions clustered, frequently sharing
>   cells (the original permits co-located units at setup). Edge staging
>   applies only to later reinforcements. On arrival the original also
...
- ~~the `combat_readiness` numerator~~ — RESOLVED by the audit (see the second
  ...
  instead: the pressure inflow rate, out-of-contact decay, retreat-direction
  order, and the break path at pressure ≥ strength;
